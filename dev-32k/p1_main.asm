...
J47D6:  CALL    INIHRD			; initialize hardware disk driver
	...
	NOP				; Mod: patch to remove MSX2 check "ret z"
        ...
        RET     M			; yep, quit
	JP      NZ,J4865		; not the disk interface starting the disk system initialization (Mod: jr-->jp)
...
J4B1B:  LD      SP,TMPSTK		; switch to temporary stack
        ...
        JP      NZ,J4B7B		; error, start DiskBASIC without running BASIC program file (Mod: jr-->jp)
        LD      C,45H			; function = close file handle
        CALL    BDOS			; execute BDOS function
        JP      J4B7F			; start DiskBASIC (Mod: jr-->jp)

; warm start DiskBASIC

J4B50:  LD      A,(WBOOT)
        CP      0C3H			; start DiskBASIC from MSXDOS ?
        JP      NZ,J4B7B		; nope, start DiskBASIC without running BASIC program file (Mod: jr-->jp)
        LD      HL,DBUF
        LD      B,(HL)
        INC     B
...
J6AA3:  LD      (SP_BDOS),SP		; store stack pointer
        LD      SP,(BDOS_ST)		; switch to BDOS stack pointer
