...
; + Added 16-bit MAXENT and FASTALLOC for improved FAT16 support
...
FATSWAP1	equ	$F1D2			; FAT swap value (old: 0xFD09)
FATSWAP2	equ	$F1D4			; FAT swap current drive flag (old: 0xFFD9)
FATSWAP3	equ	$F1D5			; FAT swap block number in buffer (old: 0xFFDA)
FATSWAP4	equ	$F1D6			; FAT swap common buffer indicator (old: 0xFFDB)
F16LOSEC	equ	HOOKBE			; FAT16 32-bit sector number bit 0..15 (old: 0xFD0D)
F16HISEC	equ	HOOKBE+2		; FAT16 32-bit sector number bit 16..31 (only bit 16..23 used) (old: 0xFD0F)

; MSXDOS.SYS entry points
...
; ------------------------------------------------------------------------------
		...
		db	"AB"
		dw	A576F			; EXTENSION ROM INIT handler
		dw	A6576			; EXTENSION ROM CALL statement handler
		dw	0
		dw	0
		defs	6,0
...
; Input:  IX = bios call, others depends on the bios call
; Output: depends on the bios call
A40AB:		push	iy
		ld	iy,(EXPTBL-1+0)
		call	CALSLT
		ei
...
; Input:  H = data, E = nibblenumber
; Output: E = updated nibblenumber (+2)
A4160:		xor	a
		ld	l,8
A4163:		rlc	h
		adc	a,a
...
; Input:  E = nibblenumber+1
; Output: E = updated nibblenumber (-2), A = data
A41AD:		xor	a
		call	A41B5
		add	a,a
		add	a,a
...
; Input:  HL = cluster number, IX = pointer to DPB
; Output: HL = cluster entry content, Zx set if entry is free, DE = pointer to FAT buffer
; A41F4
GETFAT: 	ld	e,(ix+19)
...
; Input:  HL = clusternumber, DE = pointer to FAT buffer, BC = clusterentry content
A4221: 
PutFAT:
	IFDEF FAT16DOS1
		jp	F16P02
...
; Input:  HL = pointer to buffer, B = size
; Output: Zx set if equal
A424F:		ld	de,NAME1

; Subroutine compare
; Input:  DE = pointer to buffer1, HL = pointer to buffer2, B = size
; Output: Zx set if equal
A4252:		ld	a,(de)
		cp	(hl)
		inc	hl
...
A44AE:		ld	(de),a			; devicecode / direntry high byte
		inc	de
...
; Output: A = number of FATs, DE = first FAT sector, B = number sectors per FAT, HL = pointer to FAT buffer
; A45FA
...
; Input:  IX = pointer to DPB, A = relative dirsector, DE = first dirsector
; Output: DE = dirsector, B = 1, HL = pointer to dirsector buffer
A460D:		add	a,(ix+17)
		ld	e,a
		ld	d,(ix+18)
		jr	nc,A4617
...
; Input:  DEBC = recordnumber, HL = number of records
; Output: DEHL = last record done, BC = number of records done
...
; Input:  HL = cluster, A = relative sector in cluster
...
; A4F5E
ResvFATentry:	push	hl			; store cluster number
		push	de			; store
...
A505D:		xor	a
		...
		ld	d,(ix+15)
	IFDEF FASTALLOC
		ld	h,d			; start with last cluster
		ld	l,e
...
; Remark: activated by input 127 or 255 chars, press CTRL-J, press BS or LEFT
A5244:		xor	a
		ld	(CRLFPO),a
		push	bc
		ld	b,16
...
T5374:		db	006H,07FH,008H,00DH,00AH,015H,00BH,00CH
		db	01BH,012H,018H,01CH,01DH,01EH,01FH,001H
		db	008H

		dw	A513B
		dw	A511E,A5280,A5166,A51AB,A52AE,A5295,A5232,A5166
		dw	A5284,A5265,A5166,A517A,A518D,A51AB,A529D,A5104
TKEYNT:
...
; Input:  A=0FFH for console input, A<>0FFH for console output
; Output: A=input (console input)
...
T547D:		db	0EDH,08CH,093H,0EFH,00CH,088H,039H,040H
...
T5534:		db	200,166,200,165,200,165,200,165
...
; Input:  HL = address of FCB+1,DE = destination
...
T5696:		db	080H,09AH,"E" ,'A' ,08EH,'A' ,08FH,080H
...
T570D:		dw	A40A7,A5445,A53A7,A546E,A5474,A5465,A5454,A5462
		dw	A544E,PRTBUF,A50E0,A543C,A41EF,A509F,A50D5,A4462
		dw	A456F,A4FB8,A5006,A436C,A4775,A477D,A461D,A4392
		dw	A504E,A50C4,A5058,A505D,A56D0,A56D0,A56D0,A56D0
		dw	A56D0,A4788,A4793,A501E,A50C8,A56D0,A47BE,A47B2
		dw	A47D1,A56D0,A553C,A5552,A55DB,A55E6,A55FF,A46BA
		dw	A4720
...
A57CD:		ld	(hl),a
	...
	IFDEF FAT16DOS1
		; HOOKBE is only used to indicate beginning of DOS hooks, 
		; reuse these 3 bytes for FAT16 24-bit sector number variables
		ld	hl,HOOKBE+3
		ld	b,066H
	ELSE
		ld	hl,HOOKBE
...
A58A8:		call	A622D			; hook H_LOPD when H_CLEA is hooked (for register system bottom)
	...
	IFDEF IDEDOS1
		; relocate STROUT routine
		ld	hl,16
		call	A5EC8			; allocate memory (adjust BASIC areapointers, halt when error)
		ld	(SPRTBUF+1),hl		; patch jump address
		ex	de,hl
		ld	hl,PH_STROUT
		ld	bc,LSTROUT
		ldir				; load code in allocated memory
	ENDIF	
		ld	hl,CHAR_16
		ld	de,KANJTA
		ld	bc,4
...
J5A36:		ld	(hl),a
		...
		ld	(hl),$3B		; LOW 0x003B
		inc	hl
		ld	(hl),$00		; HIGH 0x003B update "store and change secondary slotregister" routine
		ld	hl,T63BA
		call	A6306			; relocate
...
I5B0F:		db	RDSLT % 256,C63F4-C63F4
		db	WRSLT % 256,C6415-C63F4
		db	CALLF % 256,C6443-C6415
		db	CALSLT % 256,C6455-C6443
		db	ENASLT % 256,J649C-C6455
		db	0

T5B1A:		db	0,"AUTOEXECBAS",0
L5B1A   	EQU	$-T5B1A

T5B27:		db	"RUN\"AUTOEXEC.BAS",0
L5B27   	EQU	$-T5B27

T5B38:		dw	A5B92			; start DiskBASIC in direct mode
...
A5BA0:		ld	sp,0C200H		; switch to a temporary stack
		...
		db	13,10
	IFDEF IDEDOS1
		db	"Disk BASIC version 1.0",MOD1,MOD2,13,10,0
	ELSE
		db	"Disk BASIC version 1.0",13,10,0
	ENDIF
		ld	hl,NTSTOP
...
T5C96:		dw	H_DSKO,A6B96
		...
		dw	H_NODE,A737C
		dw	H_ERRP,A71D3
		dw	H_PHYD,PhyDiskIO	; A6055
		dw	H_FORM,A60B0
		dw	BDOSBO,A56D3
		dw	0

T5D24:		db	1,048H,'-',1,041H,'-',1,047H,"):",0
T5D2F:		db	"M-D-Y):",0
T5D37:		db	"D-M-Y):",0
...
A5D48:		call	PrintMsg
		db	13,10,"Enter date (",0
	IFNDEF IDEDOS1
		defs	5,0			; room for expansion 
	ENDIF
		ld	a,(COUNTR)
		cp	1
		ld	hl,T5D24
...
T5ED0:		
	IFDEF IDEDOS1
		db	"Not enough memory for IDE",0
	ELSE
		db	"No enough memory",0
		defs	5,0			  ; room for expansion 
	ENDIF
		di
		halt
...
; Input:  HL = number of bytes to allocate
; Output: Cx set if out of memory
A5EE8:		ld	a,l
		or	h			; size = 0 ?
		ret	z			; yep, quit
		xor	a
...
; Output: A = slotid
A5F95:		ld	b,6
		db	021H			 ; LD HL,xxxx (skips next instruction)
...
; Input:  A = driveid, B = number of sectors, C = mediadescriptor, DE = start sector, HL = transferaddress
; A6052
ReadSector_all:	
		and	a
		db	038H			 ; JR C,xx (skips next instruction)
...
PhyDiskIO:	
	IFDEF FAT16DOS1
		...
		and	$7f			; make sure that 16-bit flag is reset
		ld	c,a
r60551: 	pop     af
	ENDIF
		push	ix
...
; Input:  A=driveid
; Output: IYH=slotid diskdriver, H=slotid diskdriver, A=local driveid diskdriver
; GetDiskSlot
...
; Input:  Cx reset = use free BASIC memory as formatbuffer, Cx set = use specified buffer, HL = start of buffer, BC = size of buffer
...
T61AD:		db	"Write protected",0
T61BD:		db	"Not ready",0
T61C7:		db	"Disk error",0
T61D2:		db	"Bad parameter",0
T61E0:		db	"Format complete",0
...
T6248:		dw	T61AD			; dskfmt error 0
		dw	T61BD			; dskfmt error 2
		dw	T61C7			; dskfmt error 4
		dw	T61C7			; dskfmt error 6
		dw	T61C7			; dskfmt error 8
		dw	T61C7			; dskfmt error 10
		dw	T61D2			; dskfmt error 12
		dw	T5ED0			; dskfmt error 14
		dw	T61C7			; dskfmt error 16
...
L625A:		ld	a,(TARGET)		; driveid
		...
		push	af
		call	PrintMsg
		db	13,10,"Insert diskette for drive ",0
	IFNDEF IDEDOS1
		defs	10,0			; room for expansion
	ENDIF
		pop	af
		call	A408F			; output to screen
		call	PrintMsg
		db	':',13,10,"and strike a key when ready",0
	IFNDEF IDEDOS1
		defs	10,0			; room for expansion
	ENDIF
...
T62ED:		ld	a,d
		...
		rst	R_CALLF
		db	000H
		dw	A6AA7			; call BLOAD for diskdevice
		push	hl
...
T63BA:		
	IFDEF IDEDOS1
		dw	C63F4-C63F4	; 0000H
		dw	R0003-C63F4	; 0003H
		dw	J6404-C63F4	; 0010H
		dw	R0013-C63F4	; 0013H
		dw	R0017-C63F4	; 0017H
		dw	R001C-C63F4	; 001CH
		dw	R0022-C63F4	; 0022H
		dw	R0025-C63F4	; 0025H
		dw	J6425-C63F4	; 0031H
		dw	R0034-C63F4	; 0034H
		dw	R0039-C63F4	; 0039H
		dw	R003F-C63F4	; 003FH	
		dw	R0069-C63F4	; 0068H + 1 (modified undocumented instruction)
		dw	R006C-C63F4	; 006BH + 1
		dw	R0074-C63F4	; 0073H + 1
		dw	J646C-C63F4	; 0087H	+ 1
		dw	R007B-C63F4	; 008AH + 1
		dw	R007E-C63F4	; 008DH + 1
		dw	R0094-C63F4	; 00A3H + 1
		dw	J649C-C63F4	; 00B7H + 1
		dw	R00AB-C63F4	; 00BAH + 1
		dw	J64A9-C63F4	; 00C4H + 1
		dw	R00B8-C63F4	; 00C7H + 1
		dw	R00BC-C63F4	; 00CBH + 1
		dw	J651E-C63F4	; 0139H	+ 1
		dw	R0131-C63F4	; 0140H + 1
		dw	J652C-C63F4	; 0147H + 1
		dw	R0145-C63F4	; 0154H + 1
		dw	C6541-C63F4	; 015CH + 1
	ELSE
		dw	C63F4-C63F4
		dw	R0003-C63F4 
		dw	R0017-C63F4 
...
J6436:		pop	bc
		...
		and	3FH
		or	c
		call	$0046			; restore secondary slotregister
		pop	af
		pop	hl
		ret
...
R0094:		call	C6455			; MSXDOS CALSLT
		...
		or	c
		di
		call	$004B			; restore secondary slotregister
		ld	(hl),e
		ex	af,af'
		exx
		ret
...
C6541:		call	C6549			; switch page 0 (same primary slot)
		ld	HL,SLTTBL
		ld	(HL),D			; update secondary slotregister copy in SLTTBL (note: assumes primary slot 0!)
		RET
...
C6549:		RRCA
		RRCA
		AND	03H
		ld	D,A			  ; get secondary slot from slotid
		ld	A,(YFFFF)
		CPL
...
C655C:		OUT	(0A8H),A		; make secondary slotregister accessable (switch page 3)
		ld	A,(YFFFF)
		CPL
...
; Subroutine restore secondary slotregister (helper routine at 0046H)
		OUT	(0A8H),A		; make secondary slotregister accessable (switch page 3)
		ld	A,L
		JR	J656F			; change secondary slotregister, restore primary slotregister and quit
...
; Input:  IX = returnaddress replacement, IYH = number of bytes to move
;
...
A6CD6:		db	0F6H			; OR xx: trick to skip next instrunction
...
T6E68:		db	045H,065H,053H,060H,000H,000H,000H,000H
...
A6F9A:		ld	hl,FILNAM
		...
		call	A6FF5			; expand * wildcard
		pop	hl			; restore BASIC pointer
		db	0F6H			; OR xx: trick to skip next instrunction, clear Cx: ? character allowed
...
T72AE:		dw	A72B0			; pointer to the default diskerror handler for DiskBASIC
...
T72C2:		dw	A71AE			; Write Protect error, disk write protect error
		dw	A71B4			; Not Ready error, disk offline error
		dw	A71B1			; Data/CRC error, disk i/o error
		dw	A71B1			; Seek error, disk i/o error
		dw	A71B1			; Record not found error, disk i/o error
		dw	A71B1			; Write fault error, disk i/o error
		dw	A71B1			; Other error, disk i/o error
...
PRTBUF:		jp	PH_STROUT
		...
		dw	0			; SRCHLO16
		dw	0			; FATSWAP1
		db	$ff			; FATSWAP2 (initial value $ff)
		db	0			; FATSWAP3
		dw	0			; FATSWAP4
		db	0			; RW_16
	ELSE
...
; Input:  HL = source address, DE = destition address, BC = size
...
; ------------------------------------------------------------------------------
PH_STROUT:	call	SDOSOF			; enable ram on page 1
		ld	a,(de)
		call	SDOSON			; enable master diskrom
		inc	de
		cp	'$'
...
LastEntry:	push	hl
		...
		ld	d,0
		jr	_lastentry2
_lastentry1:	ld	e,(ix+$15)
		ld	d,(ix+$16)
_lastentry2:	sbc	hl,de			; last direntry ?
		pop	hl
		ret
	
; Subroutine get next directory entry
NextDir16:	call	LastEntry		; last direntry ?
		jp	nc,A4367		; nc=yes, update directory of disk when needed and quit
		ld	de,(LASTEN16)
		inc	de
		ld	(LASTEN16),de
		ld	a,e
...
GetDir16:	ld	hl,(ENTFRE16)
		jp	A4317			; get direntry

SearchFirst16:	ld	de,(LASTEN16)
		ld	(SRCHLO16),de
		ld	a,(LASTEN)
		ret

SearchNext16:	ld	de,(SRCHLO16)
		ld	(LASTEN16),de
		ret
...
NewAlloc:	push	de
		...
		ret	nz			; nz=yes, cutoff count
A508B:		dec	hl
		dec	de
		ld	a,e
		or	d
		jr	nz,NewAlloc		; next cluster
		ret
	ENDIF
