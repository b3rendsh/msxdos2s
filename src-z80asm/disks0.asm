...
;rem: phase 2 macro and print directive not implemented for z80asm
;_DOSCP	MACRO	X
;	IF2
;	IF	($ NE X)
...
I_BC00	EQU	0BC00H	                ; ramdisk boot sector
D_BE00  EQU     0BE00H                  ; number of ramdisk segments
ISBE02  EQU     0BE02H                  ; ramdisk segment table
...
SIRQ    equ     0F1E5H
SRDSLT  equ     0F1E8H			; RDSLT
SWRSLT  equ     0F1EBH			; WRSLT
SCALSLT equ     0F1EEH			; CALSLT
SENASLT equ     0F1F1H			; ENASLT
SCALLF  equ     0F1F4H			; CALLF
...
SPROMP  EQU     0F2DCH  ; --S--
SBDOS   EQU     0F2DEH  ; --S--
RD_ADD  EQU     0F2E1H  ; --SL-
RD_SNU  EQU     0F2E3H  ; --SL-
RD_SCN  EQU     0F2E5H  ; --SL-
IX_BDO  EQU     0F2E6H  ; ---L-
SP_BDO  EQU     0F2E8H  ; --SL-
BLDCHK  EQU     0F2F1H  ; --S--
BSVCHK  EQU     0F2F3H  ; --S--
BUF_1   EQU     0F2F5H  ; --SL-
BUF_2   EQU     0F2F7H  ; --SL-
...
C403C:  XOR     A			; bank = 0
        CALL    C4092                  	; select DOS2 ROM bank
        JP      J47D6
...
I4049:  PUSH    AF
        LD      A,(D40FF)
        PUSH    AF                      ; store current DOS2 ROM bank
        XOR     A			; bank = 0
        CALL    C4092                  	; select DOS2 ROM bank
        CALL    C4CF3                  	; H_TIMI handler
        POP     AF			; restore DOS2 ROM bank
        CALL    C4092                  	; select DOS2 ROM bank
        POP     AF
        RET
...
C4092:  BNKCHG
        ...
        DEFS    040B4H-$-S_ORG0,0

;         Subroutine EXTBIO handler
;            Inputs  ________________________
;            Outputs ________________________

I40B4:  CALL    C40E7                   ; H_EXTBIO MSX-JE
        JP      C410C                   ; H_EXTBIO DOS2

;         Subroutine H_TIMI handler
;            Inputs  ________________________
;            Outputs ________________________

I40BA:  CALL    C40EA                   ; H_TIMI MSX-JE
        JP      C4CF3                   ; H_TIMI DOS2
...
I413A:
	...
	DEFB    "Disk BASIC version 2.01",0
        DEFB    "Copyright (C) 1989 ASCII Corporation",0

	ELSE

	DEFB    "Disk BASIC version 2.00",0
...
J49AE:  LD      (HL),0C9H
        ...
        LD      HL,I4D32
        LD      (SPROMP),HL		; address of prompt subroutine
        LD      HL,I6A82
...
J4A24:  INC     C			; next drive id
        ...
        LD      DE,I404D		; interrupt handler MSX-JE + DOS2
        ...
        LD      DE,I4043		; EXTBIO handler MSX-JE + DOS2
...
J6A86:  EX      AF,AF'
        ...
        JR      C,J6AB4			; yep,
J6AA3:  LD      (SP_BDO),SP		; store stack pointer
        LD      SP,(BDOS_S)		; switch to BDOS stack pointer
        CALL    C6AB9			; execute BDOS function
        LD      SP,(SP_BDO)		; restore stack pointer
        OR      A			; update Zx flag
        RET
...
J6D49:  EI 
        ...
        LD      (RD_ADD),HL		; store transfer address
        LD      A,B
...
J6E11:  CALL    GET_P2                  ; GET_P2
        ...
        LD      E,H
        LD      D,0
        LD      HL,ISBE02               ; ramdisk segment table
        ADD     HL,DE
        ADD     HL,DE
        CALL    GET_P2                  ; GET_P2
        PUSH    AF			; store current segment page 2
...
; DRIVER section starts here
        ...
        DEFS	08000H-$-S_ORG0,0FFH	; rem: filler is 0FFF not 00H
