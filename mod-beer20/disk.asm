...
; The disk section of this DOS 1.03 version, without the BEERxx modifications,
...
IFDEF BEER20
	; Additional routines used by the disk-ide module
	PUBLIC	GETFAT			; A41F4
	PUBLIC	GetFATbuf		; A45FA
	PUBLIC	DSK_abs_read		; A46C5
	PUBLIC	DSK_abs_write		; A4755
	PUBLIC	OldGetFAT		; A41FA+3
	PUBLIC	PutFAT			; A4221
	PUBLIC	ReadSector_all		; A6052
	PUBLIC	GetDPBptr		; A4563+3 
	PUBLIC	DPB_change_entry	; A4529
...
DATABA	equ	0F1C9H			; start of fixed diskvars
...
DIRTYD  equ     0F23CH                  ; directory buffer dirty flag
DMAADD  equ     0F23DH                  ; transferaddress
...
DIRBFD  equ     0F246H                  ; driveid of sector in directory buffer
CURDRV  equ     0F247H                  ; default driveid
DAY     equ     0F248H                  ; current day (1..31)
MONTH   equ     0F249H                  ; current month (1..12)
YEAR    equ     0F24AH                  ; current year (offset to 1980)
DAYCNT  equ     0F24CH                  ; current days since 1-1-1980
WEEKDA  equ     0F24EH                  ; current day of week (0=sunday)

HOOKBE	EQU	0F24FH
H_PROM  equ     0F24FH                  ; prompt for disk hook
H_UNPA  equ     0F252H                  ; get fat entry content hook
H_DEVN  equ     0F255H                  ; check if devicename hook
H_CONT  equ     0F258H                  ; try next direntry hook
H_GETE  equ     0F25BH                  ; get next direntry hook
H_NEXT  equ     0F25EH                  ; next direntry hook
H_MOVN  equ     0F261H                  ; validate FCB drive and filename hook
H_DOOP  equ     0F264H                  ; fcb open hook
H_FATR  equ     0F267H                  ; get latest FAT hook
H_GETI  equ     0F26AH                  ; get pointer to DPB of current drive hook
...
LASTEN  equ     0F2B8H                  ; current direntry number
NAME1   equ     0F2B9H                  ; filename1
ATTRIB  equ     0F2C4H                  ; original DR byte FCB
NAME2   equ     0F2C5H                  ; filename2 (rename)
NAME3   equ     0F2D0H                  ; temporary save for NAME1 and ATTRIB (rename)
CREATI  equ     0F2DCH                  ; flag ignore fileattributes
SECCLU  equ     0F2DDH                  ; current relative sector in cluster
DSKERR  equ     0F2DEH                  ; result of recordoperation
TRANS   equ     0F2DFH                  ; flag increase current relative sector in cluster (0 means not)
PREREA  equ     0F2E0H                  ; flag flake read (0 means real read)
THISDR  equ     0F2E1H                  ; current driveid
NEXTAD  equ     0F2E2H                  ; transferaddress for recordoperation
RECPOS  equ     0F2E4H                  ; start record (32 bit) for recordoperation
RECCNT  equ     0F2E8H                  ; number of records for recordoperation
LASTPO  equ     0F2EAH                  ; current relative cluster of file
CLUSNU  equ     0F2ECH                  ; current cluster of file
...
ENTFRE  equ     0F2FEH                  ; first free direntry (0FFH if none found)
READOP  equ     0F2FFH                  ; flag diskoperation (0 if read, 1 if write)
CONTPO  equ     0F300H                  ; pointer to remaining lineinput from CON read record operation
MAXCLS  equ     0F302H                  ; temporary store for maximium cluster
                                        ; looks like F304-F305 are unused!
CPMCAL  equ     0F306H                  ; flag CP/M compatible BDOS call (0 means no CP/M, HL has value, <>0 means CP/M, HL is compatible filled)
...
COUNTR  equ     0F30EH                  ; date format (0 japanese, 1 european, 2 american)
KANJTA  equ     0F30FH                  ; double byte header char table
FUTURE  equ     0F313H                  ; looks like F313-F322 are unused
DISKVE  equ     0F323H                  ; diskerror handler pointer
BREAKV  equ     0F325H                  ; abort handler pointer
AUXBOD  equ     0F327H                  ; AUX input hook (MSXHOOK style), default returns CTRL-Z in register A
...
TIMFLG  equ     0F338H                  ; use clockchip flag
F_SPSA  equ     0F339H                  ; saved stackpointer format routine
CURDAT  equ     0F33BH                  ; days since 1-1-1980, used when no clockchip
RECSIZ  equ     0F33DH                  ; recordsize GET/PUT recordoperations
TARGET  equ     0F33FH                  ; at systeminit: CTRL key flag, later: saved driveid driveroperation (0=A:)
NOTFIR  equ     0F340H                  ; flag kernel cold boot (0 = cold, <>0 = warm)
...
DOSFLG  equ     0F346H                  ; flag MSXDOS has been running (0 = no MSXDOS yet)
SNUMDR  equ     0F347H                  ; number of drives in disksystem
MASTER  equ     0F348H                  ; slotid disksytem rom
HIMSAV  equ     0F349H                  ; disksystem bottom (lowest address used by the disksystem)
DOSHIM  equ     0F34BH                  ; msxdos system bottom
SSECBUF equ     0F34DH                  ; pointer to sectorbuffer, can be used by the disk hardware driver
SBUFFE  equ     0F34FH                  ; pointer to datasectorbuffer
SDIRBU  equ     0F351H                  ; pointer to directorysectorbuffer
FCBBAS  equ     0F353H                  ; pointer to the diskbasic FCB's
SDPBLI  equ     0F355H                  ; DPB table for 8 drives
RSLREG  equ     0F365H                  ; routine read primary slotregister
SDOSON  equ     0F368H                  ; routine enable disksystem rom on page 1
SDOSOF  equ     0F36BH                  ; routine enable dos ram on page 1
XFER    equ     0F36EH                  ; routine transfer to/from dos ram
SAUXIN  equ     0F371H                  ; auxiliary input routine
SAUXOU  equ     0F374H                  ; auxiliary output routine
BLDCHK  equ     0F377H                  ; routine diskbasic BLOAD
BSVCHK  equ     0F37AH                  ; routine diskbasic BSAVE
BDOS    equ     0F37DH                  ; BDOS entry point
...
IFDEF BEER20
Q41C1:  defb    " MSX-DOS IDE v2.0.5a based on SOLiD IDE v1.9  "
ELSE
...
IFDEF BEER19_OLD
	; The beer ide v1.9 interface will always take control as the master disk system.
ELSE
...
; diskinterface then drives C to F will be moved to A to D (by repositioning the DRVTABLE values). 
; This won't work properly if there are more interfaces in the system than the FDD and this IDE.
;
; Floppy - Part 1 of 3
r271:	ld	a,(DRVTBL)
	ld	c,a
...
r275:	ld	(de),a
	...
	pop	af
	ld	b,a
	ld	hl,SDPBLI+4		; DPBTBL pointer to 3d drive (C:)
r283a:	push	bc			; copy some default DPBs for DOS2 to work properly
	push	hl
	ld	hl,15h
	call	A5EC8			; AllocMem
	ex	de,hl
...
;
IFNDEF BEER19_OLD
	call	INIENV			; initialize environment with partition table info
ENDIF
	ld	hl,DISKID
...
; Floppy - Part 3 of 3
	ld	hl,DRVTBL
	ld	bc,0400h		; b = max 4 disk interfaces
r281:	ld	a,(hl)			; get number of drives per interface
	or	a
	jr	z,r278
	add	a,c			; sum #drives
	ld	c,a
	...
	inc	hl
	djnz	r281
r278:	ld	a,c			; c = total number of drives for all interfaces
	or	a
	jr	nz,r280
	ld	hl,DRVTBL+2		; first interface has no drives, adjust drivetable
	ld	de,DRVTBL
	ld	bc,6
	ldir
	ld	(hl),0
	ld	hl,DRVTBL
	ld	bc,0300h		; b = now max 3 interfaces
	jr	r281			; retry counting drives

; Initialize DPBs (only used by DOS1)
r280:	ld	b,c
	ld	hl,SDPBLI		; DPB table for 8 drives
r283:	push	bc
	push	hl
	ld	a,(hl)
	inc	hl
	or	(hl)
	jr	nz,r282			; DPB initialized for drive?
	ld	hl,15h			; no, reserve mem for this drive's DPB
	call	A5EC8			; AllocMem
	ex	de,hl
	pop	hl
	push	hl
...
ENDIF
        ...
        pop     hl                      ; size of the biggest sector
IFDEF BEER20
; Beer ide will allocate only 1.5K memory space for a 3 sector FAT buffer, for all drives.
...
; FAT buffer: part 1 of 2
	...
	ld	(HIMSAV),hl
	inc 	hl
	ld	(0ffdbh),hl		; FAT common buffer (?)
ELSE
        call    A5EC8                   ; allocate memory (adjust BASIC areapointers, halt when error)
        ld      (SDIRBU),hl             ; allocate dirsector buffer
ENDIF
...
; Continue initialization with no more beer ide customization until the boot sequence is started.
        ld      hl,AUXBOD
...
A5AE7:  
IFDEF BEER20
; First try to boot from floppy (A), if not successful then boot from first partition.
; A few more boot code validations are added.
	ld	a,(DOSFLG)		
	or	a			; MSXDOS has been running?
	jr	nz,A5B56		; nz=yes
	call	GetMySlot
	ld	b,a
	ld	a,(0fb22h)
	cp	b			; Is this the first disk interface?
	scf
	jr	z,A5B66			; z=yes
A5B56:	ld	hl,(SDIRBU)
	ld	a,(CURDRV)
	...
	ld	bc,001f8h
	or	a
	call	PHYDIO			; first try to boot from floppy
A5B66:	ld	a,0ffh
	ld	(DIRBFD),a
	ld	hl,(SDIRBU)
	call	c,HDDBOOT		; if not successful, try boot HDD
	ret	c			; error, quit
	ld	de,0c000h
	ld	bc,00100h
	ldir				; copy bootloader to C000
	ld	a,(0c01eh)		; start of boot program, do some extra validation
	cp	0d0h			; starts with 'ret nc' instrution?
	jr	z,A5B85			; z=yes
	cp	018h			; start with 'jr xx' instruction?
	jr	nz,A5B8E		; nz=no
A5B85:	ld	a,(0c000h)
...
A5BA0:  ld      sp,0C200H               ; switch to a temporary stack
        ...
        defb    13,10
IFDEF BEER20
        defb    "Disk BASIC version 1.0i",13,10,0
ELSE
...
A5C16:  ld      hl,T72AE
        ...
        call    A5EB8                   ; allocate memory (adjust HIMEM, halt when error)
        ld      (BLDCHK+1),hl           ; setup BLOAD jumpentry
        ex      de,hl
        ld      hl,T62ED
        ldir                            ; copy bsave/bload patch code
        ld      hl,0FFF5H
        add     hl,de
        ld      (BSVCHK+1),hl           ; setup BSAVE jumpentry
        ld      a,(MASTER)
        ld      de,0FFF9H
        add     hl,de
        ld      (hl),a
...
C6443:  EX      (SP),HL
        ...
        PUSH    DE
        LD      A,(HL)
        PUSH    AF			; beer19: undocumentend z80 instruction 'LD YH,A'
        POP     IY			; "
        INC     HL
        LD      E,(HL)
        INC     HL
...
A70FA:  call    A716C
        ...
        ld      de,BUF+121
        call    A461D                   ; create file
        and     a			; beer19: 'or a' same result (z-flag)
        jp      nz,A71AB
        ld      l,a
        ld      h,a
        ld      (BUF+117),hl
        ...
        ld      (BUF+154),hl
        ld      (BUF+156),hl
        inc     hl			; beer19: 'inc l' same result (hl=0 --> hl=1)
        ld      (BUF+98),hl
        ld      (BUF+135),hl
        pop     hl
A7128:  push    hl
